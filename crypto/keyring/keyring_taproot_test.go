...
func TestKeyManagementKeyRingForTaproot(t *testing.T) {

	conf := sdk.GetConfig()
	conf.SetBech32PrefixForAccount("bc", "bcoper")
	conf.Seal()
	...
	require.NoError(t, err)
	addr, err = sdk.AccAddressFromBech32("bc1p5cyxnuxmeuwuvkwfem96lqzszd02n6xdcjrs20cac6yqjjwudpxqkedrcr")
	require.NoError(t, err)
	_, err = kb.KeyByAddress(addr)
	require.Error(t, err)
...
func TestTaprootKeyring(t *testing.T) {
	sdk.GetConfig().SetBech32PrefixForAccount("bc", "bcoper")

	cdc := getCodec()
	tempDir := t.TempDir()
	kb, _ := New("keybasename", "test", tempDir, nil, cdc)

	addr, err := sdk.AccAddressFromBech32("bc1p5cyxnuxmeuwuvkwfem96lqzszd02n6xdcjrs20cac6yqjjwudpxqkedrcr")
	require.NoError(t, err)
	t.Log("addr:", hex.EncodeToString(addr))
